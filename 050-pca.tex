...
class PCA1D:
    ...
    def normalize_u(self, eps=1e-8):
        nrm = (sum(v.data * v.data for v in self.u) + eps) ** 0.5
        for ui in self.u:
            ui.data /= nrm
...
\toprule
\textbf{Car} & \textbf{Type} & \makecell{\textbf{kWh/}\\\textbf{100 km}} & \textbf{HP} & \makecell{\textbf{0--100 km/h}\\\textbf{(s)}} & \makecell{\textbf{Trunk}\\\textbf{(L)}} \\
\midrule
...
\begin{exercise}{Kaj spremeni standardizacija?}
Iste podatke analiziraj s PCA pred standardizacijo in po njej, nato primerjaj smeri glavnih komponent in pojasnjeno varianco ter razloži, kateri atributi prevladajo, kadar so njihove merske skale zelo različne.
\end{exercise}
...
\begin{exercise}{Kje so značilke v prostoru glavnih komponent?}
Vsako značilko predstavi z njenim enotskim vektorjem v izvornem prostoru, vektorje projiciraj na prvi dve glavni komponenti in jih skupaj s primeri izriši v ravnini PCA; iz smeri in dolžine projiciranih vektorjev razloži, katere značilke so povezane s posamezno komponento, katere značilke kažejo v podobne oziroma nasprotne smeri in katere v prvih dveh komponentah skoraj niso zastopane.
\end{exercise}

\begin{exercise}{Kdaj glavna komponenta sledi šumu?}
Sestavi podatke z dvema skupinama točk: na začetku naj bosta dobro ločeni v vodoravni smeri, nato postopoma povečuj navpični šum. Za vsako stopnjo šuma izračunaj prvo glavno komponento in jo primerjaj z intuitivno smerjo ločevanja skupin. Opazuj, pri kateri stopnji šuma prva komponenta preneha slediti ločevanju in začne slediti šumu. Poskusi ustvariti primer, kjer PCA popolnoma zgreši intuitivno ``najpomembnejšo'' smer podatkov.
\end{exercise}
    
\begin{exercise}{Zakaj PCA ne vidi kroga?}
Generiraj točke na krožnici ali polkrogu. Izračunaj PCA in podatke projiciraj na prvo komponento. Rezultat vizualiziraj ter razloži, zakaj PCA kljub očitni geometrijski strukturi ne najde uporabne enodimenzionalne predstavitve.
\end{exercise}
...
\begin{exercise}{Koliko variance pojasni prva komponenta pri naključnih podatkih?}
Ustvari podatkovno množico z desetimi atributi, kjer so vse vrednosti vzorčene neodvisno iz standardne normalne porazdelitve. Atribute standardiziraj in izračunaj PCA. Izmeri delež variance, ki ga pojasni prva komponenta, ter ga primerjaj s teoretično vrednostjo $1/10$. Poskus ponovi za različne velikosti podatkovne množice (npr.\ 20, 50, 100, 500 in 5000 primerov) in za vsako velikost izvedi več ponovitev. Nariši graf povprečne pojasnjene variance prve komponente v odvisnosti od velikosti vzorca. Ali se pri večjih množicah pojasnjena varianca približuje pričakovani vrednosti? Kako se spreminja razlika med prvo in drugo komponento?
\end{exercise}

\begin{exercise}{Kako stabilne so glavne komponente?}
Iz iste podatkovne množice večkrat vzorči nekoliko drugačne učne množice, vsakič izračunaj PCA ter primerjaj smeri prvih komponent, da oceniš, pri katerih podatkih so komponente stabilne in kdaj že majhna sprememba vzorca povzroči velik zasuk.
\end{exercise}
...
\begin{exercise}{Ali PCA odkrije skupine povezanih atributov?}
Sestavi podatkovno množico z devetimi atributi: naj prvi trije izhajajo iz iste skrite spremenljivke, drugi trije iz druge in zadnji trije iz tretje; vsakemu atributu dodaj manjšo količino naključnega šuma. Podatke standardiziraj in izračunaj PCA. Za prve tri glavne komponente izpiši uteži posameznih atributov ter jih prikaži z grafom. Preveri, ali imajo atributi iz iste skupine podobne uteži na isti komponenti. Nato povečaj količino šuma in opazuj, kako se spreminjata delež pojasnjene variance in interpretabilnost komponent.
\end{exercise}
...
\begin{exercise}{PCA z razredno informacijo.}
Izberi podatkovno množico za klasifikacijo in kriterijski funkciji PCA dodaj člen, ki nagrajuje večje razdalje med središči posameznih razredov v projekciji; nato postopoma povečuj utež tega člena ter spremljaj, kako se spreminjajo smeri glavnih komponent, delež pojasnjene variance in ločenost razredov v prostoru prvih dveh komponent.
\end{exercise}
...
\begin{exercise}{Pojasnjena varianca PCA na testni množici.}
Na internetu poišči podatkovni nabor z vsaj petimi zveznimi atributi in vsaj 200 primeri (npr.\ Wine, Parkinsons, Auto MPG, Abalone ali podoben nabor iz UCI Machine Learning Repository). Podatke standardiziraj in jih večkrat naključno razdeli na učno in testno množico. PCA nauči le na učni množici, nato na testni izračunaj delež variance, ki ga pojasnijo prva, prvi dve in prve tri glavne komponente. Poskus ponovi za velikosti učne množice 10, 20, 50, 100 in 200 ter rezultate povpreči. Nariši odvisnost pojasnjene variance na testni množici od velikosti učne množice in razloži opažene razlike. Ali se pojasnjena varianca s povečevanjem učne množice približa tisti, ki jo dobiš, če PCA naučiš na vseh podatkih?
\end{exercise}
...
\begin{exercise}{Analitični PCA in gradientni sestop.}
Po korakih iz zgornje izpeljave implementiraj analitično PCA: podatke standardiziraj, izračunaj kovariančno matriko ter poišči lastne vektorje in lastne vrednosti. Rezultate (smeri glavnih komponent, uteži atributov in delež pojasnjene variance) primerjaj z implementacijo prek gradientnega sestopa (\texttt{PCA1D} ali \texttt{PCA2D}) na izbranih podatkih iz poglavja ali na lastni sintetični ali neki iz omrežja pridobljeni množici. Ali se oba pristopa ujemata? Kje se morebiti razlikujeta? Primerjaj tudi hitrosti postopkov.
\end{exercise}