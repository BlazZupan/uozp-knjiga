...
Mera $R^2$ je pogosto bolj primerna od mer, kot je RMSE, ker je brezrazsežna in vedno v enakem razponu, torej med 0 in 1,
%
\marginnote{Pa je to res? Je možno, da bi kakšen model napovedoval slabše od najenostavnejšega modela? Bo to tako na učni množici primerov ali na novih primerih?}
%
ter zato omogoča primerjavo točnosti med različnimi podatkovnimi množicami ali problemi. Medtem ko RMSE podaja napako v enotah ciljne spremenljivke, $R^2$ izraža, kolikšen delež variance podatkov pojasni model, kar je pogosto bolj intuitivno. Poleg tega $R^2$ neposredno primerja model z osnovnim pristopom napovedovanja s povprečjem, zato takoj pove, ali model dejansko prinaša izboljšavo.

\begin{exercise}{Negativen $R^2$ na testni množici.}
Pripravi majhno univariatno učno množico, jo razdeli na učno (npr. 70\%) in testno, in neposredno iz učne množice zgradi model direktno ali pa s polinomsko razširitvijo različnih stopenj. Napovedi ovrednoti na testnih podatkih z lastno implementacijo $R^2$. Poišči primer delitve podatkov ali stopnje polinoma, pri katerem je testni $R^2$ negativen. Zakaj model v tem primeru napoveduje slabše od konstantnega modela, ki bi vrnil povprečje učnih ciljnih vrednosti?
\end{exercise}

\begin{exercise}{Negativen $R^2$ pri napovedih s srednjo vrednostjo.}
    Podobno kot zgoraj, le da napoveduješ s srednjo vrednostjo odvisne spremenljivke učne množice. Pričakovana vrednost $R^2$ na testni množici je 0. Kaj v resnici dobiš? Preveri, kako je to odvisno od velikosti učnih množic.
\end{exercise}
...
\begin{exercise}{Regularizacijska pot in nestabilnost izbora značilk.}
Ustvari sintetično regresijsko množico podatkov z nekaj resnično informativnimi značilkami, več nepomembnimi in skupino močno koreliranih kopij ene pomembne značilke. \emph{Regularizacijska pot} je zaporedje ocen uteži pri naraščajočih vrednostih $\lambda$; za vsako $\lambda$ nauči model z regularizacijo L1 ali L2 ter izriši odvisnosti vrednosti uteži za posamezne atribute v odvisnosti od stopnje regularizacije. Pri L1 spremljaj, katere uteži postanejo ničelne in kdaj. Poskus ponovi z vsaj tremi različnimi podatkovnimi množicami.
\end{exercise}

\begin{exercise}{Korelirane značilke.}
Ustvari sintetično regresijsko množico podatkov, pri kateri je ciljna spremenljivka odvisna od treh atributov. V podatke dodaj več atributov, ki so močno korelirani s posameznim izbranim atributom iz originalne množice. Nauči model z L1 regularizacijo pri izbrani stopnji regularizacije $\lambda$. Učenje ponovi večkrat zapored (npr. z različnimi naključnimi začetnimi utežmi). Preveri, ali Lasso med koreliranimi značilkami vedno izbere isto. Kaj ti ugotovitve povejo o razlagi modela z ničelnimi utežmi?
\end{exercise}

\begin{exercise}{Koliko podatkov potrebujemo za zanesljiv izbor značilk?}
Na sintetičnih podatkih z nekaj pomembnimi in veliko nepomembnimi značilkami (recimo, 3 pomembne in 50 naključnih značilk) ponovi L1-regulariziran izbor pri različnih velikostih učne množice ter spremljaj, kako pogosto model izbere prave značilke.
\end{exercise}
...
Podatke smo, kot je razvidno iz zgornjega, pred učenjem tudi standardizirali. Tovrstna normalizacija podatkov nam omogoča hitrejšo konvergenco gradientnega sestopa, saj so vse značilke na po velikosti primerljive in je prostor optimizacije bolj enakomernen, kar preprečuje cik-cakasto napredovanje in pospeši približevanje minimumu.

\begin{exercise}{Ko je pomembna značilka na napačni skali.}
Ustvari podatke, kjer imajo atributi zelo različne merske lestvice, primerjaj L1 in L2 regularizacijo pred standardizacijo in po njej ter razloži, zakaj regularizacija brez ustreznega skaliranja lahko neenakovredno kaznuje uteži.
\end{exercise}

Sledi poskus, kjer model gradimo pri različnih stopnjah regularizacije in izpisujemo točnost tako na učni kot na testni množici:
...
\begin{exercise}{Preveč regularizacije.}
Na isti učni in testni množici postopoma povečuj $\lambda$ od skoraj nič do zelo velike vrednosti ter izriši učni in testni $R^2$, število neničelnih uteži in poišči območje, kjer regularizacija pomaga, ter točko, kjer začne model zaradi prevelike poenostavitve izgubljati napovedno moč.
\end{exercise}
...
Zgornjo delitev množice podatkov uporabimo za oceno točnosti postopka učenja. To je torej točnost, ki jo ocenimo na testni množici. Rabimo pa poročati o končnem modelu. Ta bi bil lahko tisti, ki je bil zgrajen s parametri, ki so vodili do najboljše točnosti na validacijski množici najboljši. A tak model je bil naučen le na učni množici in zato ne izkorišča vseh razpoložljivih podatkov. Za gradnjo končnega modela lahko zato izhajamo iz vseh podatkov, te tokrat delimo samo na učno in validacijsko množico in ponovimo celotni postopek izbora stopnje regulacije.

Delitev na učno in valicijsko množico pri zadnjem koraku je potrebna zato, ker naš postopek gradnje modela temelji na taki delitvi in posledično oceni primerih meta-parametrov učenja. Da ponovimo: postopek gradnje našega modela tokrat ni samo linearna regresija, ampak vključuje tudi korake izbora stopnje regularizacije. Model, ki ga na koncu želimo uporabljati oziroma o katerem bomo poročali torej ni samo rezultat ene optimizacije, ampak rezultat celotnega postopka, ki vključuje tudi izbor stopnje regularizacije. Ta postopek pa po definiciji potrebuje delitev na učno in validacijsko množico.
...
\begin{exercise}{Uhajanje informacij pri standardizaciji in izboru $\lambda$.}
Uporabi podatke o stopnji telesne maščobe iz poglavja, a jih omeji na majhno množico (npr.\ prvih 50 primerov). Iz nje loči majhno učno množico (npr.\ 10 primerov); testna množica so vsi preostali primeri (npr.\ 40). Primerjaj dva postopka: (1) nepravilnega, kjer standardiziraš vseh 50 primerov pred delitvijo, izbereš $\lambda$ z najvišjim $R^2$ na testni množici in to vrednost poročaš kot oceno uspešnosti; (2) pravilnega, kjer najprej ločiš učno in testno množico, iz učnih primerov ločiš še validacijsko množico (npr.\ 3), standardiziraš le z učnih podatkov, $\lambda$ izbereš na validacijski množici in točnost enkrat oceniš na testni množici. Primerjaj testne ocene $R^2$ in razloži, zakaj mora biti tudi izbira regularizacije biti izvedena na učni množici.
\end{exercise}
