...
\begin{exercise}{Kaj naredi osamelec?}
Ustvari podatke za univariatno linearno regresijo, nato enemu primeru\\
postopoma povečuj odstopanje od ostalih ter opazuj, kako se spreminjajo\\
naučena premica, parametra $w$ in $b$ ter vrednost funkcije izgube.
\end{exercise}

\begin{exercise}{Stopnja učenja in konvergenca.}
Uporabi iste podatke kot v poglavju (\texttt{random.seed(42)}, \texttt{n=5}, \texttt{ys = [2*x - 1 + ...]}). Za vsako stopnjo učenja iz množice $\{0.001,\ 0.01,\ 0.1,\ 1.0,\ 10.0\}$ zaženi \texttt{train(LinReg(), xs, ys, learning\_rate=..., n\_epochs=200)} in zabeleži končne vrednosti $w$, $b$ ter morebitno napako. Določi največjo stopnjo učenja, pri kateri učenje še konvergira blizu $w \approx 2$ in $b \approx -1$, ter najmanjšo, pri kateri pride do prekoračitve ali očitne divergenc. Na kratko razloži, zakaj prevelik korak gradientnega sestopa pri tej izgubi destabilizira učenje.
\end{exercise}

\begin{exercise}{Pot gradientnega sestopa.}
Uporabi iste podatke kot v poglavju. Prilagodi funkcijo \texttt{train} tako, da po vsaki epohi zabeleži par $(w, b)$. Za isti začetni model zaženi učenje s \texttt{n\_epochs=100} pri stopnjah učenja $0.001$ in $0.1$, nato v ravnini $(w, b)$ izriši obe poti (točke poveži v zaporedju). Opiši, zakaj majhna stopnja učenja vodi do gladke poti proti minimumu, prevelika pa do cik-cak gibanja čez dolino izgube.
\end{exercise}

\begin{exercise}{Kritična stopnja učenja.}
Za neke podatke primerne za linearno regresijo, na primer podobnim tem v poglavju, začni pri konvergentni stopnji učenja (npr.\ \texttt{0.001}) in jo iterativno podvajaj, dokler učenje z \texttt{n\_epochs=200} ne divergira (dobiš numerično napako, izguba se znatno poveča). Nato z binarnim iskanjem zoži interval med zadnjo stabilno in prvo nestabilno vrednostjo ter oceni največjo stabilno stopnjo učenja.
\end{exercise}
...
\begin{exercise}{Velikost paketa in konvergenca.}
    Uporabi podatke iz tega poglavja
    (\texttt{random.seed(42)}, \texttt{n=1000},
    \texttt{ys = [2*x + 1 + random.gauss(0,5) for x in xs]}).
    Za vsako velikost paketa iz množice
    $\{1,\ 5,\ 10,\ 50,\ 100,\ 1000\}$
    zaženi učenje z
    \texttt{learning\_rate=0.01}
    in \texttt{n\_epochs=500}.
    Zabeleži končne vrednosti parametrov modela ter izgubo na celotni učni množici.
    Primerjaj tudi potek izgube med učenjem.
    Katera velikost paketa povzroči najbolj nestanovitno učenje?
    Katera konvergira najhitreje?
    Zakaj majhni paketi vodijo do bolj šumnih ocen gradienta.
\end{exercise}

\begin{exercise}{Pot paketnega učenja.}
Uporabi podatke za univariatno regresijo. Prilagodi funkcijo \texttt{train} za paketno učenje (\texttt{batch\_loss}, \texttt{batch\_size=10}) tako, da po vsaki epohi zabeleži par $(w, b)$. Za isti začetni model zaženi učenje s \texttt{n\_epochs=200} pri stopnjah učenja $0.001$ in $0.1$, nato v ravnini $(w, b)$ izriši obe poti. Kaj opaziš?
\end{exercise}
...
\begin{exercise}{Ista premica, drugačne merske skale\\ spremenljivk.}
Iste podatke za multivariatno regresijo\\
predstavi enkrat v prvotnih enotah\\
in drugič tako, da en atribut pomnožiš z velikim faktorjem,\\
nato primerjaj hitrost gradientnega sestopa, velikosti naučenih uteži\\
in napovedi obeh modelov.
\end{exercise}
...
\begin{exercise}{Vpliv šuma na oceno uteži.}
Ustvari več podatkovnih množic po postopku iz poglavja (\texttt{random.seed(42)}, \texttt{n=1000}, enak \texttt{X}, različne vrednosti standardnega odklona šuma pri generiranju odvisne spremenljivke $y$, npr.\ $\{0,\ 1,\ 5,\ 20\}$). Za vsako množico nauči model \texttt{LinReg(n\_inputs=3)} in primerjaj ocenjene uteži ter prosti člen s pravimi vrednostmi ($w_0=2$, $w_1=3$, $w_2=-1$, $b=1$). Kako natančno model pri različnih ravneh šuma oceni prave vrednosti uteži?
\end{exercise}
...
\begin{exercise}{Pomembni in nepomembni atributi.}
Ustvari podatke po postopku iz multivariatnega primera (\texttt{random.seed(42)}, \texttt{n=1000}, $y$ odvisen le od prvih treh atributov). Podatkom dodaj pet naključnih atributov, ki niso povezanih z $y$. Nauči model \texttt{LinReg(n\_inputs=8)} in atribute razvrsti glede na absolutno vrednost ocenjenih uteži. Preveri, ali so med tremi najvišje uvrščenimi res prvi trije atributi. Kaj opaziš pri preostalih?
\end{exercise}

\begin{exercise}{Kdaj linearni model ni dovolj?}
Generiraj podatke iz preproste nelinearne zveze, na primer $y=x^2+\epsilon$,\\
nanje nauči linearni model ter z grafom napovedi in ostankov pokaži,\\
kako lahko iz strukture napak prepoznaš, da izbrana oblika modela\\
ni ustrezna.
\end{exercise}
