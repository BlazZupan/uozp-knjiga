...
\begin{exercise}{Katero ime je bolj verjetno? Primerjava bigramskega modela in nevronske mreže.}
Nauči bigramski model in model z nevronsko mrežo. Sestavi testno množico imen, 
kjer lahko pomešaš slovenska in neka tuja imena, nato z obema modeloma 
izračunaj povprečno negativno log-verjetnost posameznega imena. Imena razvrsti 
od najbolj do najmanj verjetnih ter primerjaj, pri katerih imenih se modela
najbolj razlikujeta in zakaj.
\end{exercise}
...
\begin{exercise}{Vpliv velikosti konteksta na točnost modela.}
Nauči modele, ki naslednji znak napovedujejo iz enega, dveh, treh in več
predhodnih znakov ter primerjaj njihovo točnost na učnih in testnih podatkih.
\end{exercise}
...
\begin{exercise}{Kako so si podobni pari črk?}
Nauči trigramski model, ki iz dveh predhodnih znakov napoveduje naslednjega,
nato za vsak par črk vzemi vektor napovedanih verjetnosti naslednjega znaka,
izračunaj razdalje med temi vektorji ter pare z MDS vloži v ravnino.
Na karti poišči skupine parov s podobnimi nadaljevanji in razloži
njihovo jezikovno podobnost.
\end{exercise}
...
def generate_name(g):
    ...
    while True:
        ...
        h = torch.tanh(emb.view(1, block_size * embedding_dim) \
                       @ W1 + b1)
        logits = h @ W2 + b2
