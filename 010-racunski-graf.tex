...
\runinheading{Analitična rešitev} Ker je dana funkcija enostavna, kvadratna, lahko njen minimum poiščemo analitično. Funkcija bo imela ekstrem v točki, kjer je prvi odvod enak nič. Odvajamo našo funkcijo:  
\[
\frac{d}{da} f(a) = 2a - 10
\]
%
in njen odvod enačimo z nič:  
\[
2a - 10 = 0
\]
Rešimo za \( a \):  
...
\]
%
in posodobimo vrednost parametra,
%
\[
...
\]
%
in ponovno posodobimo njegovo vrednost,
%
\[
...
\]
%
in posodobimo vrednost parametra $a$, 
%
\[
...
\begin{exercise}{Korak pri končnih diferencah.}
Za funkcijo
\[
f(a)=a^2-10a+28
\]
izračunaj odvod v točki $a=6$ z metodo končnih diferenc. Preizkusi vrednosti $h=10^{-1},10^{-2},10^{-4},10^{-8}$ in rezultate primerjaj z analitičnim odvodom. Opiši, pri katerih vrednostih $h$ je ocena najboljša in kdaj se začnejo pojavljati numerične težave.
\end{exercise}
...
\begin{exercise}{Gradientni spust.}
Obravnavaj funkcijo
\[
f(x)=x^4-3x^3+2.
\]
Napiši program, ki z metodo gradientnega spusta poišče lokalni minimum funkcije na intervalu $[0,3]$. Odvod izračunaj numerično s končnimi diferencami.
\end{exercise}
...
\begin{figure*}
  \centering{\includegraphics[width=0.9\textwidth]{figs/010-racunski-graf.pdf}}
  \caption[6pt]{}
...
\begin{exercise}{Gradient s končnimi diferencami.}
Za funkcijo
\[
L(a, b, c, d) = (ab + c) d
\]
in vrednosti $a=2$, $b=-3$, $c=10$, $d=-2$ napiši program, ki z metodo končnih diferenc oceni vse štiri parcialne odvode. Rezultate primerjaj z analitičnim gradientom $\nabla L = (6, -4, -2, 4)$ in kratko opiši, kako se numerične ocene razlikujejo od analitičnih vrednosti.
\end{exercise}
