...
\begin{exercise}{Operacija \texttt{tanh}.}
V razred \texttt{Value} dodaj metodo \texttt{tanh}, ki na enak način kot \texttt{\_\_add\_\_} in \texttt{\_\_mul\_\_} ustvari novo vozlišče z vrednostjo $\tanh(\texttt{self.data})$, si zapomni predhodnika in oznako operacije. Nato zgradi graf za izraz \texttt{(a + b).tanh() * c} z vrednostmi $a=1.0$, $b=0.5$, $c=2.0$, izpiši končno vrednost in preveri, da ima izhodno vozlišče pravilne predhodnike v \texttt{\_prev}.
\end{exercise}
...
\begin{figure*}
\includegraphics[width=\linewidth]{figs/020-računski-graf.pdf}
\end{figure*}
...
\begin{figure*}
\includegraphics[width=\linewidth]{figs/020-računski-graf-odvodi.pdf}
\end{figure*}

\begin{exercise}{Vzvratni prehod za \texttt{tanh}.}
Razširi metodo \texttt{tanh} v razredu \texttt{Value} z implementacijo \texttt{\_backward()} (odvod $\tanh(x)$ je $1-\tanh^2(x)$). Zgradi graf za izraz \texttt{(a + b).tanh() * c} z vrednostmi $a=1.0$, $b=0.5$, $c=2.0$, po \texttt{backward()} izpiši gradiente po \texttt{a}, \texttt{b} in \texttt{c}. Nato z metodo končnih diferenc numerično oceni iste parcialne odvode in primerjaj rezultate; če se razlikujejo, poišči napako v forward ali backward delu.
\end{exercise}
...
\begin{exercise}{Preverjanje gradientov.}
Napiši splošno funkcijo, ki za poljuben izraz iz \texttt{Value} primerja gradiente, pridobljene z \texttt{backward()}, z numeričnimi ocenami s končnimi diferencami. Preiskusi jo na osnovni implementaciji razreda in na lastnih razširitvah.
\end{exercise}

\begin{exercise}{Gradientni sestop v dveh spremenljivkah.}
Poišči minimum funkcije
\[
f(a,b)=a^2+b^2+ab-6a-4b
\]
z gradientnim sestopom. Parametra $a$ in $b$ predstavi z vozlišči \texttt{Value}, začni z $a=0$, $b=0$, naredi 50 korakov s hitrostjo učenja $0.1$ (v vsakem koraku zgradi graf, pokliči \texttt{f.backward()}, posodobi \texttt{a.data} in \texttt{b.data}). Izpiši končne vrednosti $a$, $b$ in $f(a,b)$. Analitično reši $\partial f/\partial a=0$ in $\partial f/\partial b=0$ ter preveri, ali se rezultat gradientnega sestopa ujema. Na koncu numerično oceni parcialna odvoda v končni točki z metodo končnih diferenc in primerjaj z \texttt{a.grad} in \texttt{b.grad}.
\end{exercise}
...
\begin{exercise}{Ko se spremenljivka pojavi večkrat.}
Zgradi več računskih grafov, v katerih se ista spremenljivka pojavi na več mestih (na primer $f(a)=a^2+a^3+2a$) ter preveri in razloži, zakaj se morajo prispevki različnih poti pri vzvratnem prehodu sešteti.
\end{exercise}