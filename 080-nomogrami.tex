...
Dobljeni model lahko predstavimo z nomogramom (slika~\ref{fig:bled-nomogram}).
...
\begin{exercise}{Kdaj Poissonova regresija premaga linearno?}
Generiraj podatke, kjer je ciljna spremenljivka nenegativno celo število in njena pričakovana vrednost narašča eksponentno z eno ali več značilkami, nato na istih podatkih nauči linearno in Poissonovo regresijo ter primerjaj njune napovedi, napake in obnašanje pri večjih vrednostih ciljne spremenljivke.
\end{exercise}
...
\begin{exercise}{Nomogram nariši sam.}
Za naučen model linearne ali logistične regresije iz njegovih uteži samodejno zgradi nomogram (sestavi primerno knjižnico v programu Python), nato na nekaj izbranih primerih preveri, ali ročno seštevanje točk na nomogramu vrne enake napovedi kot model.
\end{exercise}
